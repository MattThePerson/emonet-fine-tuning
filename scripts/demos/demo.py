...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        print(f"Predicted Emotion {emotion_classes[int(predicted_emotion_class)]} - valence {output['valence'].clamp(-1.0,1.0).cpu().item():.3f} - arousal {output['arousal'].clamp(-1.0,1.0).cpu().item():.3f}")
